...
local survivalGame = import('/maps/aeon_illuminate_survival.v0011/src/SurvivalGame.lua')
...
Survival_SpawnUnit = function(UnitID, ArmyID, POS, OrderID) -- blueprint, army, position, order
	...
	if (UnitID == "UAL0205") then
		NewUnit:CreateShield({
			...
			RegenAssistMult = 60,
			ShieldEnergyDrainRechargeTime = 60,
			ShieldMaxHealth = 2300 * ScenarioInfo.Options.opt_Survival_HealthMultiplier,
			ShieldRechargeTime = 20,
			ShieldRegenRate = 60,
			ShieldRegenStartTime = 1,
			ShieldSize = 15,
